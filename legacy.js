//https://raw.githubusercontent.com/flamescape/SourceQuery/master/sourcequery.js
    ...
    var send = function(buffer, responseCode, cb) {
        ...
        sq.client.send(buffer, 0, buffer.length, sq.port, sq.address, function(err, bytes){
            ...
            var relayResponse = function(buffer, remote){
                if (buffer.length < 1) {
                    console.log("Got empty buffer");
                    return;
                }
                
                if (bp.unpack("<s", buffer)[0] !== responseCode) {
                    console.log("Response code mismatch, expected ", responseCode, " but got ", bp.unpack("<s", buffer)[0]);
                    return;
                }
